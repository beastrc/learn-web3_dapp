...
const CheckAccountId: React.FC<CheckAccountIdT> = ({ network, freeAccountId, setFreeAccountId, setIsFreeAccountId }) => {
    ...
    const onInputChange = (e: any) => {
        setAlertStatus('info')
        setAlertMsg('Enter an account name including the .testnet suffix -> "example.testnet"')
    ...
    return (
        <Space direction="vertical" size="small">
            <Text style={{ fontWeight: "bold", fontSize: 20}}>Choose an account identifier:</Text>
            <Space direction="horizontal" size="small">
                <Input placeholder="choose-your-account-name.testnet" onChange={onInputChange} style={{ width: "500px" }} />
                <Button type="primary" onClick={checkAvailabilityOfAccountId} loading={isFetching} disabled={ validAccountId }>Check it!</Button>
            </Space>
