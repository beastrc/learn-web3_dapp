import Connect from '@polka/components/steps/Connect';
import Account from '@polka/components/steps/Account';
import Balance from '@polka/components/steps/Balance';
import Transfer from '@polka/components/steps/Transfer';
import Restore from '@polka/components/steps/Restore';
import Estimate from '@polka/components/steps/Estimate';
import Deposit from '@polka/components/steps/Deposit';

export {Connect, Account, Balance, Transfer, Restore, Estimate, Deposit};
